...
addpath C:\Users\becahp\UnB\Artigo\Simulacoes\TFET2\02_otimizacao\01_vpg\Adapt
...
op2 =  rdcelpa('Adapt/vpg_adapt_bg_0_dd_iv.elpa','*',[],op2);
op2 =  rdcelpa('Adapt/vpg_adapt_bg_1_dd_iv.elpa','*',[],op2);
...
legend({'vpg = |1.0|V','vpg = |1.0|V, vbg=0V','vpg = |1.0|V, vbg=+1V', 'vpg=adapt, vbg=0V', 'vpg=adapt, vbg=+1V'}, 'Location', 'best');
...
figure
h(1) = semilogy(op2(4).V_g,op2(4).I_d );
xlabel('V_{G}, V_{PG}=4V_{G}-1V (V)')
ylabel('I_{d} (A)')
axis([0 0.5 4E-16 4e-5])
ay = gca;
ay.YTick = [1e-15 1e-13 1e-11 1e-9 1e-7 1e-5];
hold; 
h(2) = semilogy(op2(5).V_g,op2(5).I_d);

h(1).Color = map(1,:);
h(2).Color = map(2,:);
legend({'VBG=0V','VBG=1V'}, 'Location', 'best');
